...
write_HIVMMER_Command=function(sample_num, output_name=default_name, expname="", sample_namesdb, datapath) {
  ...
  file1=paste0(expname,"/filtered_", sample_num, "_S", sample_num, "_L001_R1_001.fastq.gz")
  file2=paste0(expname, "/filtered_", sample_num, "_S", sample_num, "_L001_R2_001.fastq.gz")
...
fileConn<-file("O:/JOLabShared/SaraDrescher/Code/HIVMMERCommands_nano_filtered.txt")
writeLines(c(noquote("#!/bin/sh"), noquote("cd /home/sdresche/JOLabShared/SaraDrescher/Molecular_clock"), write_HIVMMER_Command(1:length(sample_names$Sample_Name), expname="nano/filtered", sample_namesdb=sample_names, datapath=currentpath)), fileConn)
close(fileConn)

#awk '{ sub("\r$", ""); print }'  ~/JOLabShared/SaraDrescher/Code/HIVMMERCommands_nano_filtered.txt> ~/JOLabShared/SaraDrescher/Code/HIVMMER_nano_filtered.sh
