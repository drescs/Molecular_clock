...
#note that the blast command requires merged files
  ...
  for (i in (7:18)){
    ...
    path="~/JOLabShared/SaraDrescher/Molecular_clock/nano/"
  ...
  files_list=as.data.frame(cbind(cons_names[7:18,2], as.character(cons_names[7:18,1]), forward_list, reverse_list))
  ...
  write_lines(c(noquote("#!/bin/sh"), noquote("cd ~/JOLabShared/SaraDrescher/Molecular_clock/nano/"), noquote("module load BLAST+"), noquote ("module load blast"), commands), "O:/JOLabShared/SaraDrescher/Code/pt_cons_blastcommands_nano_LAIQ23.txt")
...
path="O:/JOLabShared/SaraDrescher/Molecular_clock/nano/"
totals=vector()
not_contam=vector()
for (j in 7:18){
  i=j-6
