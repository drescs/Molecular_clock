...
filtered1=read.csv("C:\\Users\\Sara\\Documents\\HHMI_projects\\Molecular_clock\\Analysis\\third_output_filtered2.csv")
filtered1$Fragment=filtered1$V6


filtered2=read.csv("C:\\Users\\Sara\\Documents\\HHMI_projects\\Molecular_clock\\Analysis\\third_output_second_run_filtered.csv")


filtered_noVA1=filtered1[which(!is.na(filtered1$Month)),]
...
filtered_all=rbind(filtered_noVA1[,c("Sample_name", "APD", "Month", "Fragment")], filtered_noVA2[,c("Sample_name", "APD", "Month", "Fragment")])

pdf("O:/JOLabShared/SaraDrescher/Molecular_clock/some_plots.pdf")
ggplot(filtered_noVA1, aes(y=APD, x=Month)) + geom_point() +  geom_smooth(method = "lm") +  stat_regline_equation(label.x = 3, label.y = .015) + ggtitle ("Exp 1--filtered")
...
ggplot(filtered_all, aes(y=APD, x=Month)) + geom_point() +  geom_smooth(method = "lm") +  stat_regline_equation(label.x = 3, label.y = .015) + ggtitle ("All--filtered")


ggplot(filtered_noVA1, aes(y=APD, x=Month, color=Fragment, fill=Fragment)) + geom_point(aes(color=Fragment)) + geom_smooth(method = "lm", se=T) + ggtitle ("Exp 1--filtered") + facet_grid(col=vars(Fragment))
ggplot(filtered_noVA2, aes(y=APD, x=Month, color=Fragment, fill=Fragment)) + geom_point(aes(color=Fragment)) + geom_smooth(method = "lm", se=T) + ggtitle ("Exp 2--filtered") + facet_grid(col=vars(Fragment))
ggplot(filtered_all, aes(y=APD, x=Month, color=Fragment, fill=Fragment)) + geom_point(aes(color=Fragment)) + geom_smooth(method = "lm", se=T) + ggtitle ("All--filtered") + facet_grid(col=vars(Fragment))
...
ggplot(filtered_all, aes(y=APD, x=Month, color=Sample_name, fill=Sample_name)) + geom_point(aes(color=Sample_name)) + geom_smooth(method = "lm", se=F) + ggtitle ("All, by Fragment") + facet_grid(col=vars(Fragment))
