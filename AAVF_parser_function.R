...
AAVF.parser=function(filepath, fragment, codon_variant_frac_min=0.01, coverage_min=5000) {
  library(readr)
  library(stringr)
  library(magrittr)
  library(tidyverse)
  ...
  my_output_vector=c(my_output_vector, HXB_nt_start, HXB_nt_end)
  #plot coverage by positionls
covplot=ggplot(POS_COVERAGE, aes(y=COVERAGE, x=POS)) + 
  geom_bar(position="dodge", stat="identity") +  
  theme_bw()

out_put_list=list(my_output_vector, covplot)
